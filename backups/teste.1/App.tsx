...
const App: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // 4. Atividades, Voos, Hotéis, Carros (Fallback directly to constants for now if needed)
      // Note: We can add specific endpoints later if they don't exist yet
      setActivities(getActivities(language));
      setFlights(getFlights(language));
      setHotels(getHotels(language));
      setCars(getCars(language));
    } catch (error) {
  ...
  const handleCheckOut = async (resId: string, restaurantId: string, tableId: string) => {
    ...
    const rest = restaurants.find(r => r.id === restaurantId);
    if (rest) {
      const updatedTables = rest.tables?.map(t => t.id === tableId ? { ...t, status: 'available' as const, customerName: undefined, reservationTime: undefined } : t);
      ...
      try {
        ...
        alert('✅ Saída registada! Ganhou 50 Créditos Azores4You.');
  ...
  const handleReview = async (reviewData: any) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/restaurants/${reviewData.restaurantId}/reviews`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...reviewData,
      ...
      if (response.ok) {
        setMyReservations(prev => prev.map(res => 
          res.id === reviewData.reservationId ? { ...res, reviewed: true } : res
        ));
        fetchData();
      }
    } catch (err) {
