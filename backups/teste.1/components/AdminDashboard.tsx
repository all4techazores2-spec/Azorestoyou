...
type Tab = 'restaurants' | 'activities' | 'flights' | 'hotels' | 'cars' | 'buses' | 'accounts';
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full z-20 overflow-y-auto">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <div className="pt-4 mt-4 border-t border-slate-800">
            <button onClick={() => { setActiveTab('accounts'); setEditingItem(null); }} className={`w-full text-left p-3 rounded-xl flex items-center gap-3 ${activeTab === 'accounts' ? 'bg-emerald-600' : 'hover:bg-slate-800'}`}>
              <Lock className="w-5 h-5" /> Gerir Contas
            </button>
          </div>
        </nav>
