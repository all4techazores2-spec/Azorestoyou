...
interface RestaurantModalProps {
  ...
  onReserveSuccess?: (resId: string, restName: string, restId: string) => void;
}
...
const RestaurantModal: React.FC<RestaurantModalProps> = ({ 
  ...
  const [customerEmail, setCustomerEmail] = useState('traveler@azores4you.com');
  const [customerPhone, setCustomerPhone] = useState('+351 912 345 678');
  const [customerName, setCustomerName] = useState('Cliente Viajante');
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('✅ Reserva enviada com sucesso ao restaurante!');
      ...
      // Créditos
      if (setUserCredits) {
        let finalCredits = userCredits + 2;
        if (paymentType === 'points') {
          const totalCost = orderItems.reduce((acc, item) => acc + (item.quantity * 10), 0);
          finalCredits -= totalCost;
        }
