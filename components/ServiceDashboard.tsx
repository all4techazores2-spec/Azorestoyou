...
export const ServiceDashboard: React.FC<ServiceDashboardProps> = ({
  ...
  // Mobile First Navigation tabs: 'inicio', 'agenda', 'novo_trabalho', 'mensagens', 'mais'
  // On desktop/tablet, this syncs with sidebar options.
  const [mobileTab, setMobileTab] = useState<string>('inicio');
  const [activeTab, setActiveTab] = useState<string>('dashboard'); // For desktop alignment
  ...
  const getWorkflowTotal = () => {
    const servicePrice = 65; // Base fee
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans antialiased text-slate-800 pb-20">
      ...
      <div className="p-4 space-y-6 flex-1 max-w-lg mx-auto w-full">
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            {activeJob ? (
              <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                ...
                <div className="space-y-1">
                  <h3 className="text-lg font-black text-slate-800">{activeJob.clientName}</h3>
        ...
        {/* Mais Tab View (Lists all other management pages in clean mobile list) */}
        {mobileTab === 'mais' && (
      ...
      <nav className="fixed bottom-0 inset-x-0 h-20 bg-white/90 backdrop-blur-lg border-t border-slate-150 z-40 flex items-center justify-between px-6 shadow-[0_-8px_30px_rgba(0,0,0,0.06)]">
        ...
        <button 
          ...
          className="w-14 h-14 bg-gradient-to-tr from-blue-600 to-emerald-500 rounded-full flex items-center justify-center text-white shadow-lg shadow-blue-500/20 active:scale-90 transition-transform -translate-y-4 border-4 border-slate-50 relative z-50"
        >
          <Plus size={24} />
        </button>
      ...
      {/* ── NOVO TRABALHO SLIDE DRAWER / MODAL ── */}
      <AnimatePresence>
        {showNewJobDrawer && (
                    ...
                    {['Antes', 'Durante', 'Depois'].map(categ => (
                      <div key={categ} className="space-y-1 text-center">
                        ...
                        <div className="aspect-square bg-slate-100 border border-slate-200 rounded-2xl overflow-hidden relative">
                          <img src={jobPhotos[categ.toLowerCase() as 'antes' | 'durante' | 'depois'][0]} alt="Job State" className="w-full h-full object-cover" />
                        </div>
                      </div>
                    ))}
                  </div>
