...
# Mapeamento de IDs do utilizador para IDs do db.json (se necessário)
# No db.json temos "trail_salto_cabrito", etc.
mapping = {
    "PR03SMI_vista_rei_sete_cidades": "trail_vista_rei", # Presumindo que vamos usar o ID curto ou criar se não existir
...
# Inverter o mapeamento para facilitar a busca no db.json
# Mas na verdade o utilizador quer injetar estes dados.
...
try:
    ...
    for code, data in dados_poi.items():
        # Tentar encontrar o trilho no db.json pelo ID mapeado ou pelo título que contenha o código
        found = False
        target_id = mapping.get(code)
        
        for act in activities:
            if act.get('id') == target_id or code in act.get('id', '') or code in act.get('title', ''):
                act['pontosInteresse'] = data['pontosInteresse']
                # Garantir que o ID seja o código correto se estivermos a atualizar
                act['id'] = code 
                found = True
                break
        
        if not found:
            # Se não encontrar, vamos criar um novo entrada de trilho básica
            # para que o Nicolau possa editar depois
            new_trail = {
                "id": code,
    ...
    print(f"❌ Erro ao atualizar db.json: {str(e)}")
